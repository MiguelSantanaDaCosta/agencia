...
public class AgenciaService {
    ...
    public EstatisticasDTO obterEstatisticas() {
        Double faturamento = agenciaRepository.calcularFaturamentoTotal().orElse(0.0);
        return new EstatisticasDTO(
                agenciaRepository.countClientes(),
                agenciaRepository.countViagens(),
                agenciaRepository.countPacotes(),
                faturamento,
                agenciaRepository.countClientesAtivos(),
                agenciaRepository.countViagensDisponiveis()
        );
    }

    @Transactional(readOnly = true)
    public EstatisticasDTO obterEstatisticasPeriodo(java.time.LocalDate inicio, java.time.LocalDate fim) {
        Double faturamentoPeriodo = agenciaRepository.calcularFaturamentoPeriodo(inicio, fim).orElse(0.0);
        return new EstatisticasDTO(
                agenciaRepository.countClientesAtivos(),
                agenciaRepository.countViagensDisponiveis(),
                agenciaRepository.countPacotes(),
